...
alonzoMainnet = either (error . (<>) "alonzoMainnet: parse failed: " . show) id $ do
  hsh <- C.deserialiseFromRawBytesHex "8959c0323b94cc670afe44222ab8b4e72cfcad3b5ab665f334bbe642dc6e9ef4"
...
recent = either (error . (<>) "recent: parse failed: " . show) id $ do
  hsh <- C.deserialiseFromRawBytesHex "22e8b5455623ff8e75a79499bada55c3b0b259db33ddf4b94efaac858a40c4ed"
...
lessRecent = either (error . (<>) "recent: parse failed: " . show) id $ do
  hsh <- C.deserialiseFromRawBytesHex "bee4dff659bdd2091343c23e10d4bc9f9c74e8aa4c805bf18ccb3f779c79e787"
